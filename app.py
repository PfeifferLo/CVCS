...
with tab1:
    ...
    for _, row in map_data.iterrows():
        ...
        firma = row["Zugehoerigkeit"] if "Zugehoerigkeit" in row and pd.notna(row["Zugehoerigkeit"]) else "k.A."
        wert  = round(row[variable], 2) if pd.notna(row[variable]) else "k.A."

        popup_text = (
            "<b>Firma:</b> " + str(firma) + "<br>"
            "<b>Variable:</b> " + str(variable) + "<br>"
            "<b>Wert:</b> " + str(wert)
        )
